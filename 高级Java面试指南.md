...
>
> 例：面试官好，我叫XXX，17年毕业于烟台大学软件工程专业，大四实习到19年三月在一家互联网金融公司做P2P项目，主要负责银行存管，用户中心和标的管理相关的开发。后来入职酒仙网参与新零售项目开发，目前主要负责门店和人员管理，茅台及审计数据上报相关的开发工作。公司目前的技术架构主要包括SpringBoot、SpringCloud、MyBatis、MySQL、Redis、RabbitMQ、Zookeeper等。平时我喜欢看看书，写写个人博客，最近也在学习容器相关的知识，学习之余，我喜欢弹钢琴，手绘，打羽毛球。
> 
...
- 防止SQL注入
  ...
  > 使用预编译SQL语句执行
  >
  > MyBatis中#和$的区别：$不会预编译SQL，会直接拼接一条SQL，而#会使用PrepareStatement作为变量注入到SQL中
...
- XSS跨站点脚本攻击
  ...
  > 反射型：做输入校验和替换
  >
  > 存储型：设置cookie为http-only访问方式，表示只能通过浏览器发送http请求才能操作cookie

- CSRF跨站点请求攻击

  > 触发跨站请求：cookie hash，请求必须带有cookie hash；web token，服务端生成，客户端校验
...
举例：抢茅台的时候，用户频繁请求后间歇性退出登陆

> - 快速定位
>
>   排查无法抢购的接口，发现监控报错和从Redis获取登陆Token异常情况
>
>   错误日志和监控显示无法获取到Redis链接
>
>   发现Redis Cluster Server CPU占用高，有大量登陆产生的Token Key注入
>
>   重启Redis服务，客户端重连
>
> - 深入分析
...
> - 取舍解决
>
>   Token下发能力限制，对IP封杀，添加验证码

### 你如何主导项目？

权责混合模式，以自己负责的模块为中心，向周围扩展
...
例：我已经拿到了几家中小型公司的Offer，但是体量跟贵公司来比不值一提，我还是希望能加入贵公司，实现我个人的价值。

我已经进入了XXX公司（大公司）的第X面，并且已经邀约了下一面，但是我还是更看好贵公司的发展，更利于我实现今后的职业规划。

贵公司是我面试的第一家，暂时没有拿到其他Offer，但是有XXX公司已经约了面试
